...
const SummerManifest: React.FC<SummerManifestProps> = ({ bookings, dailyEvents, onWeekSelect }) => {
  const handlePrint = () => {
    window.print();
  };
        ...
        <table className="w-full text-left border-collapse table-fixed">
          <thead>
            <tr className="bg-sky-50/50 border-b border-sky-100">
              <th className="p-8 w-24 text-sky-900 font-black uppercase tracking-widest text-[10px]">Week</th>
              {HOUSES.map(house => (
          ...
          <tbody>
            {WEEKS.map(week => (
              ...
              >
                <td className="p-8 font-black text-sky-900 bg-sky-50/30 group-hover:bg-sky-100/50 transition-colors">
                  <div className="flex flex-col">
                    <span className="text-lg tabular-nums">W{week}</span>
                    <span className="text-[9px] text-sky-400 font-bold tracking-tighter">{getWeekDateRange(week)}</span>
                  </div>
                </td>
                {HOUSES.map(house => {
                  const booking = bookings.find(b => b.week === week && b.houseId === house.id);
                ...
                <td className="p-8 align-top">
                   <div className="space-y-2">
                      {dailyEvents.filter(e => e.week === week).sort((a,b) => a.dayIndex - b.dayIndex).map(event => (
                      ...
                      {dailyEvents.filter(e => e.week === week).length === 0 && (
          ...
          >
            <div className="bg-sky-50 px-4 py-3 flex items-center justify-between border-b border-sky-100">
              <div className="flex items-center gap-3">
                <span className="text-lg font-black text-sky-900">W{week}</span>
                <span className="text-[10px] font-bold text-sky-400 uppercase tracking-tighter">{getWeekDateRange(week)}</span>
              </div>
