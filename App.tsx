...
import { WEEKS, HOUSES, FAMILIES, DAYS_OF_WEEK, YEARS, getWeekDateRange, getWeekShortLabel, getSeasonYear } from './constants';
...
const App: React.FC = () => {
  ...
  const totalPeopleForWeek = selectedWeek 
    ? bookings
        .filter(b => b.week === selectedWeek && b.year === selectedYear)
        .reduce((sum, b) => {
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <nav className="sticky top-0 z-50 bg-sky-950 text-white shadow-2xl px-8 py-6 flex items-center justify-between border-b border-sky-900 no-print">
        <div className="flex items-center gap-3 md:gap-5">
          ...
          <div className="flex items-center gap-2 bg-sky-900 border border-sky-800 rounded-2xl pl-3 pr-2 py-2 shadow-inner">
            <Calendar className="text-teal-400" size={18} />
            <label htmlFor="season-year" className="sr-only">Choose season year</label>
            <select
              id="season-year"
              value={selectedYear}
              onChange={(e) => setSelectedYear(Number(e.target.value))}
              className="bg-sky-900 text-white text-base md:text-lg font-black tracking-wide focus:outline-none cursor-pointer pr-1"
            >
              {YEARS.map(y => (
                <option key={y} value={y} className="bg-sky-900 text-white font-bold">{y}</option>
              ))}
            </select>
          </div>
        </div>
        ...
        {viewMode === 'manifest' && (
          <div className="space-y-8 animate-in fade-in duration-700">
            <div className="text-center max-w-2xl mx-auto space-y-4 no-print">
              ...
              <p className="text-teal-600 uppercase tracking-[0.3em] text-[10px] font-black">Summer {selectedYear} • Week 24 to 35 • Double-click any week to manage</p>
            </div>
            <SummerManifest bookings={bookings} dailyEvents={dailyEvents} selectedYear={selectedYear} onWeekSelect={handleWeekDrill} />
          </div>
        )}
          ...
          <>
            <div className="flex flex-col md:flex-row items-center justify-between gap-6 bg-white p-8 rounded-3xl shadow-sm border border-sky-100 no-print">
              <div className="flex items-center gap-6">
                ...
                <div>
                  <h2 className="text-4xl font-bold text-sky-900 serif-font italic">Week {selectedWeek}</h2>
                  <p className="text-teal-700 text-lg md:text-2xl font-bold mt-1">{getWeekDateRange(selectedWeek, selectedYear)}</p>
                </div>
              </div>
              <div className="flex flex-wrap justify-center gap-2">
                {WEEKS.map(week => (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center w-16 h-16 rounded-2xl font-black transition-all border-2 ${selectedWeek === week ? 'bg-sky-900 text-white border-sky-900' : 'bg-white text-sky-900 border-sky-100 hover:border-teal-300'}`}
                  >
                    <span className="text-base leading-none">{week}</span>
                    <span className={`text-[9px] font-bold mt-1 ${selectedWeek === week ? 'text-teal-200' : 'text-slate-400'}`}>{getWeekShortLabel(week, selectedYear)}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div className="bg-white rounded-3xl shadow-xl p-8 border border-sky-50">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2 mt-4">
                       {dailyEvents.filter(e => e.week === selectedWeek && e.year === selectedYear).map(ev => (
              ...
              <div className="lg:col-span-2 order-1 lg:order-2">
                <ScheduleGrid bookings={bookings} setBookings={setBookings} selectedWeek={selectedWeek} selectedYear={selectedYear} />
              </div>
            </div>
          </>
        )}
