...
const ScheduleGrid: React.FC<ScheduleGridProps> = ({ bookings, setBookings, selectedWeek }) => {
  ...
  const removeGroup = async (houseId: string, selectionId: string) => {
    const booking = bookings.find(b => b.week === selectedWeek && b.houseId === houseId);
    if (!booking) return;
  ...
  const completeRegistration = async () => {
    ...
    const existingBooking = bookings.find(b => b.week === selectedWeek && b.houseId === houseId);
    ...
    if (existingBooking) {
      ...
      const id = `${selectedWeek}-${houseId}-${Date.now()}`;
  ...
  const getBooking = (houseId: string) => bookings.find(b => b.week === selectedWeek && b.houseId === houseId);
